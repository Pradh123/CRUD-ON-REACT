  ...
  const Deletedata = (e, id) => {
    ...
    if (confirm1) {
      axios
        ...
        .catch((err) =>
          alert("data can not be deleted due to occurring error")
        );
    } else {
      navigate("/home");
    }
  ...
  return (
    <>
      {records.map((item) => (
        <div key={item.id} className=" flex h-10 my-3 w-full">
          ...
          <div className=" flex  w-2/12">
            <NavLink
              to={`/update/${item.id}`}
              className=" bg-emerald-300 text-2xl mx-2 text-red-500 rounded-lg  p-1"
            >
              <FaRegEdit />
            </NavLink>
            <button
              className=" bg-red-700 text-black rounded-lg mx-2 p-1  text-2xl"
            ...
            <NavLink
              to={`/read/${item.id}`}
              className=" bg-green-800 text-2xl mx-2 text-yellow-100 rounded-lg  p-1"
            >
              <GrView />
            </NavLink>
          </div>
      ...
      <div className="flex justify-center relative bottom-1">
        {currentPage > 1 ? (
          <NavLink
            className="bg-slate-400 rounded-s px-2 cursor-pointer"
            onClick={Prev}
          >
            prev
          </NavLink>
        ...
        {numbers.map((n, i) => (
          <NavLink
            className={` border border-solid px-2 cursor-pointer ${
              currentPage === n && "active" ? "bg-lime-700" : ""
            }`}
            onClick={() => ChangeN(n)}
          >
            {n}
        ...
        {currentPage < npage ? (
          <NavLink
            className="bg-slate-400 rounded-e px-2 cursor-pointer"
            onClick={Next}
          >
            next
          </NavLink>
